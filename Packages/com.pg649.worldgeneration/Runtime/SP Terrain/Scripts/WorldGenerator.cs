...
public class WorldGenerator {
    ...
    public static GameObject Generate(WorldGeneratorSettings settings) {
        ...
        if (SystemInfo.supportsComputeShaders) h.AverageFilterGPU(mask: tm.intermediate, numberOfRuns: 600);
        else h.AverageFilterMT(mask: tm.intermediate, numberOfRuns: 600);
        ...
        if (SystemInfo.supportsComputeShaders) h.AverageFilterGPU(mask: tm.levelsCorridors.InvertedBorderMask(6), numberOfRuns: 15);
        else h.AverageFilterMT(mask: tm.levelsCorridors.InvertedBorderMask(6), numberOfRuns: 15);
